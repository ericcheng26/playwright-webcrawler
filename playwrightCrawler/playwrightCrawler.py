...
from playwright import async_playwright, Error, TimeoutError
...
class PlaywrightCrawler:
    def __init__(self, base_url: str):
      ...
      file_formatter = logging.Formatter('%(asctime)s %(levelname)-7s %(message)s', '%Y-%m-%d %H:%M:%S')
      handlerConsoleLog = logging.StreamHandler()
      handlerConsoleLog.setFormatter(color_formatter)
      handlerFileLog = logging.FileHandler("playwrightCrawler.log", encoding="utf-8", mode="w")
    ...
    def _enqueueLinks(self, links) -> None:
        for link in links:
            ...
            hrefLink = urlunsplit(urlsplit(hrefLink)._replace(query="", fragment=""))
    ...
    async def _crawl(self) -> None:
      while True:
        ...
        try:
          ...
          if not response.ok:
            self.crawllogger.error('[{}] {}'.format(response.status, response.request.url))
            ...
            self.crawllogger.warning('[{}] {} Redirected to {}'.format(redirected.status, response.request.redirectedFrom.url, response.url))
          elif urlToCrawl != response.url:
            self.crawllogger.warning('[000] Browser change url {} to {}'.format(urlToCrawl, response.url))

          if response.ok:
            self.crawllogger.info('[{}] {}'.format(response.status, response.url))
            ...
            if headers.get('x-robots-tag') and 'nofollow' in headers.get('x-robots-tag'):
              self.crawllogger.warning('X-Robots-Tag HTTP header: nofollow on {}'.format(response.url))
            else:
              ...
              if 'nofollow' in metacontent:
                self.crawllogger.warning('Robots meta tag: nofollow on {}'.format(response.url))
              else:
          ...
          with self._lock:
            ...
            self.crawllogger.error('{} {}'.format(urlToCrawl, te.message.split('.')[0]))
        except (Error) as te:
            self.crawllogger.error('{} {}'.format(urlToCrawl, te.message.split('.')[0]))
        finally:
            await page.close()
    ...
    async def _lauch_browser(self) -> None:
        ...
        if not "".__eq__(self._settingsdict['PLAYWRIGHT_BROWSER_TYPE']):
          if self._settingsdict['PLAYWRIGHT_BROWSER_TYPE'] not in ['chromium', 'firefox', 'webkit']:
            raise RuntimeError('Invalid PLAYWRIGHT_BROWSER_TYPE configuration')
        ...
        self._context.setDefaultNavigationTimeout(self._settingsdict['PLAYWRIGHT_NAVIGATION_TIMEOUT'])
        ...
        self.crawllogger.info('[000] Starting browser with User Agent: {}'.format(getUA))
